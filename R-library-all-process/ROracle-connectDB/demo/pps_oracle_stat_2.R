...
case_stat <- function(id,start_date,gd_size,layer,method=1,file_path,move=NULL,move_cut=NULL
                      #,char="is not null"
                      ){
library(plyr)
...
con <- dbConnect(drv,"szgadba","Peptalk123"
    ...
    if(method %in% c(1,2)==FALSE) stop("ERROR:please make sure method is a number and in the vector of c(1,2)")
    ...
    cd <- dbGetQuery(con,paste("select to_date(substr(crime_time,1,8),'yyyy-mm-dd') as crime_date,ref_id,type_id,
			      ...
			      "and to_date(substr(crime_time,1,8),'yyyy-mm-dd') >=","to_date('",start_date,"','yyyy-mm-dd')",sep=" "))
    ##分班次
    ##else cd <- dbGetQuery(con,paste("select to_date(substr(crime_time,1,10),'yyyy-mm-dd hh24') as crime_date,ref_id,type_id,
    ##                          to_char(wgs84_x) wgs84_x,to_char(wgs84_y) wgs84_y from pps_crime_data where pd_id=",id,
    ##                          "and to_date(substr(crime_time,1,8),'yyyy-mm-dd') >= ","to_date('",start_date,"','yyyy-mm-dd')",sep=" "))
    ...
    fun <- function(k){
        ...
        #cd <- join(cd,gd[,c(1,7:10)],by="GRID_NUM",type="inner")
    ...
    stat <- stat[order(stat$grid_num,stat$date_str),]
    names(stat)[4] <- "crime_num"
    write.csv(stat,file=paste(file_path,id,"_",start_date,"_",gd_size,"_",layer,"_",method,".csv",sep=""),row.names=F)
    names(stat)[4] <- "layer0"
    ###清理内存
...
#                names(temp)[3:ncol(temp)] <- paste(names(tp)[3:ncol(tp)],"_move",j,sep="")
	...
	stat_move <- stat_move[which(stat_move$date_str > (min(stat_move$date_str)+max(move))),]
...
#save(stat,file=paste(out_matrix_path,out_matrix_original,".RData",sep=""))
#save(stat_move,file=paste(out_matrix_path,out_matrix_transform,".RData",sep=""))
rm(list=ls()) ####当有return时
gc(reset=TRUE)####无效、不执行语句
}

cat("#####已成功加载生成矩阵函数#####
#####可传入的参数说明：case_stat(id,start_date,gd_size,layer,method=1,file_path,move=NULL,move_cut=NULL)
#####s输出两个数据框分别是stat,stat_move,其中stat是某天某格子发生案情的记录数据,stat_move是每天每个格子向前推移一些天数后统计出的数据
  id:              派出所编号[PD_ID]
  ...
  file_path:       保存文件的路径,格式如,'c:/xx/xxx/' 或 'c:\\xx\\xxx\\'
...
#例如:crime_data <- case_stat(207,20130101,200,2,1,"Q:/orcl_pps/",move=c(2,5),move_cut=NULL,"loufeng207_200_20130101_2_move")

#狮山
#crime_data <- case_stat(6,20121201,150,2,1,"Q:/orcl_pps/",move=NULL,move_cut=c(1,2,3,4,5))
#crime_data <- case_stat(6,20121201,150,2,1,"Q:/orcl_pps/",move=c(1,3,6,10,15),move_cut=NULL)

#双塔
#crime_data <- case_stat(51,20121201,100,2,1,"Q:/orcl_pps/",move=NULL,move_cut=c(3,2,3,4,5))


#crime_data <- case_stat(51,20121201,100,2,1,"Q:/orcl_pps/",move=c(3,5,8,12,17),move_cut=NULL)

#娄葑
#crime_data <- case_stat(207,20130101,200,2,1,"Q:/project_pps_R/orcl_pps/",move=NULL,move_cut=c(3,2,3,4,5))
